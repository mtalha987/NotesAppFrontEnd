...
const FinishedScreen = () => {
  ...
  const fetchMainGoals = async () => {
    try {
      ...
      const response = await fetch(
        `https://vercel.com/mtalha987/notes-app-backend/api/main-goal?id=${user_id}`,
        {
          method: 'GET',
        },
      );
        ...
        const mainGoalsWithSubgoals = await Promise.all(
          data.map(async mainGoal => {
            const subgoalResponse = await fetch(
              `https://vercel.com/mtalha987/notes-app-backend/api/main-goal/sub-goal?id=${mainGoal.id}`,
            );
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const response = await fetch(
        `https://vercel.com/mtalha987/notes-app-backend/api/buying?id=${user_id}`,
        {
          method: 'GET',
        },
      );
